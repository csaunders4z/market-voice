...
Provides adaptive rate limiting, retry logic, and batch processing with async support
"""
import time
import random
import asyncio
...
class RateLimiter:
    ...
    async def _wait_for_rate_limit_async(self, api_name: str, base_delay: float):
        """Wait appropriate time between API calls using async sleep"""
        delay = self._get_delay(api_name, base_delay)
        ...
        if api_name in self.last_request_time:
            ...
            if time_since_last < total_delay:
                sleep_time = total_delay - time_since_last
                logger.debug(f"Rate limiting {api_name}: waiting {sleep_time:.2f}s (async)")
    ...
    async def _process_item_async(self, item: Any, process_func: Callable, api_name: str) -> Any:
        """Process a single item with async rate limiting"""
        try:
            await self._wait_for_rate_limit_async(api_name, 0.1)
            result = process_func(item)
            return result
        except Exception as e:
    ...
    async def batch_process_async(self, items: List[Any], batch_size: int, batch_delay: float, 
                                 process_func: Callable, api_name: str = "unknown", 
                                 max_consecutive_errors: int = 5) -> List[Any]:
        """Process items in batches with async rate limiting and early termination on persistent errors"""
        results = []
        total_batches = (len(items) + batch_size - 1) // batch_size
        consecutive_errors = 0
        rate_limit_errors = 0
        
        logger.info(f"Processing {len(items)} items in {total_batches} batches for {api_name} (async)")
        
        for i in range(0, len(items), batch_size):
            batch = items[i:i + batch_size]
            batch_num = (i // batch_size) + 1
            
            logger.debug(f"Processing batch {batch_num}/{total_batches} for {api_name} ({len(batch)} items) (async)")
            
            # Process batch items concurrently
            batch_tasks = []
            for item in batch:
                task = asyncio.create_task(self._process_item_async(item, process_func, api_name))
            ...
            # Check if we should stop due to persistent errors
            if consecutive_errors >= max_consecutive_errors:
                logger.error(f"Stopping {api_name} collection after {consecutive_errors} consecutive errors (async)")
                break
            
            # Check if we should stop due to rate limiting
            if rate_limit_errors >= max_consecutive_errors:
                logger.error(f"Stopping {api_name} collection after {rate_limit_errors} rate limit errors (async)")
                break
            
            # If entire batch failed, stop processing
            if batch_success_count == 0 and len(batch) > 0:
                logger.error(f"Entire batch {batch_num} failed for {api_name}. Stopping collection. (async)")
                break
            
            # Delay between batches (except for last batch)
            if i + batch_size < len(items) and consecutive_errors < max_consecutive_errors:
                logger.debug(f"Waiting {batch_delay}s between batches for {api_name} (async)")
                await asyncio.sleep(batch_delay)
        
        logger.info(f"Completed processing {len(results)}/{len(items)} items for {api_name} (async)")
        return results
